...
const payableInput = z.object({
  ...
  status: z
    .enum(["a_pagar", "em_aberto", "pago", "cancelado"])
    .default("a_pagar"),
...
const receivableInput = z.object({
  ...
  dueDate: z.string(),
  status: z
    .enum(["a_receber", "em_aberto", "recebido", "cancelado"])
    .default("a_receber"),
  category: z.string().optional(),
  costCenterId: z.number().optional(),
  isRecurring: z.boolean().default(false),
...
export const financialRouter = router({
  ...
  listPayable: protectedProcedure
    .input(
      z
        .object({
          status: z
            .enum(["a_pagar", "em_aberto", "pago", "cancelado", "all"])
            .default("all"),
    ...
    .query(async ({ input }) => {
      ...
      if (input?.status && input.status !== "all")
        conds.push(eq(accountsPayable.status, input.status));
      if (input?.costCenterId)
        conds.push(eq(accountsPayable.costCenterId, input.costCenterId));
      let rows = await db
        .select()
  ...
  summaryPayable: protectedProcedure.query(async () => {
    ...
    const sum = (s: string) =>
      rows
        .filter(r => r.status === s)
        .reduce((acc, r) => acc + parseFloat((r.amount as string) || "0"), 0);
    return {
      aPagar: sum("a_pagar"),
      emAberto: sum("em_aberto"),
      pago: sum("pago"),
    };
  }),
  ...
  listReceivable: protectedProcedure
    .input(
      z
        .object({
          status: z
            .enum(["a_receber", "em_aberto", "recebido", "cancelado", "all"])
            .default("all"),
          search: z.string().optional(),
        })
        .optional()
    ...
    .query(async ({ input }) => {
      ...
      const conds: any[] = [];
      if (input?.status && input.status !== "all")
        conds.push(eq(accountsReceivable.status, input.status));
      let rows = await db
        .select()
      ...
      if (input.entityId) {
        ...
        if (db) {
          if (input.entityType === "payable") {
            await db
              .update(accountsPayable)
              .set({ attachmentUrl: url, attachmentKey: key } as any)
              .where(eq(accountsPayable.id, input.entityId));
          } else {
            await db
              .update(accountsReceivable)
              .set({ attachmentUrl: url, attachmentKey: key } as any)
              .where(eq(accountsReceivable.id, input.entityId));
          }
        }
      }
