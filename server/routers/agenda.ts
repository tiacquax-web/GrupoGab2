...
export const agendaRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (input?.startDate)
        conditions.push(gte(agendaEvents.startAt, new Date(input.startDate)));
      if (input?.endDate)
        conditions.push(lte(agendaEvents.startAt, new Date(input.endDate)));
      return db
        .select()
        .from(agendaEvents)
        .where(and(...conditions))
        .orderBy(agendaEvents.startAt);
    }),
  create: protectedProcedure
    .input(
      z.object({
        ...
        type: z
          .enum(["reuniao", "visita", "ligacao", "prazo", "outro"])
          .default("outro"),
      })
    )
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      await db
        .update(agendaEvents)
        .set(input.data)
        .where(eq(agendaEvents.id, input.id));
      return { success: true };
    }),
