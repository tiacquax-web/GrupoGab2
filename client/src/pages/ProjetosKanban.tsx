...
export default function ProjetosKanban() {
  ...
  const { data: tasks = [], refetch: refetchTasks } =
    trpc.projects.listTasks.useQuery(
  ...
  const moveTask = (taskId: number, currentStatus: string) => {
    ...
    if (idx < PHASES.length - 1) {
      updateTaskMutation.mutate({
        id: taskId,
        status: PHASES[idx + 1].key as any,
      });
    }
  };

  const getTasksByPhase = (phase: string) =>
    (tasks as any[]).filter((t: any) => t.status === phase);

  const selectedProjectData = (projects as any[]).find(
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Acompanhe as fases e andamento das obras
          </p>
        </div>
        <div className="flex gap-2">
          {selectedProject && (
            <Dialog open={openTask} onOpenChange={setOpenTask}>
              ...
              <DialogContent className="max-w-md">
                <DialogHeader>
                  <DialogTitle>
                    Nova Tarefa — {selectedProjectData?.name}
                  </DialogTitle>
                </DialogHeader>
                <form
                  onSubmit={handleTask(onSubmitTask)}
                ...
                >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Textarea {...registerTask("description")} rows={2} />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Select
                        ...
                        onValueChange={v => setTaskValue("status", v as any)}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select
                        defaultValue="media"
                        onValueChange={v => setTaskValue("priority", v as any)}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        {...registerTask("assignedToName")}
                        placeholder="Nome"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <Label>Prazo</Label>
                      <Input {...registerTask("dueDate")} type="date" />
                    </div>
                  </div>
          ...
          <Dialog open={openNew} onOpenChange={setOpenNew}>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                <DialogTitle>Novo Projeto / Obra</DialogTitle>
              </DialogHeader>
              <form
                onSubmit={handleProject(onSubmitProject)}
                className="space-y-4 pt-2"
              >
                <div className="space-y-1.5">
                  <Label>Nome *</Label>
                  <Input
                    {...registerProject("name", { required: true })}
                    placeholder="Nome do projeto/obra"
                  />
                </div>
                <div className="space-y-1.5">
                  <Label>Descrição</Label>
                  <Textarea {...registerProject("description")} rows={2} />
                </div>
                <div className="space-y-1.5">
                  <Label>Responsável / Gestor</Label>
                  <Input
                    {...registerProject("managerName")}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input {...registerProject("endDate")} type="date" />
                  </div>
                </div>
                <div className="flex gap-2 pt-2">
                  <Button
                  ...
                  >
                    {createProjectMutation.isPending ? "Salvando..." : "Criar"}
                  </Button>
                </div>
              </form>
            </DialogContent>
      ...
      {selectedProject ? (
          ...
          {PHASES.map((phase, phaseIndex) => {
            ...
            return (
              <div key={phase.key} className="space-y-3">
                <div
                  className={`flex items-center justify-between p-3 rounded-lg border-t-2 bg-card ${phase.color}`}
                >
                  <span className="font-semibold text-sm">{phase.label}</span>
                  <Badge className={`text-xs ${phase.badge}`}>
                    {phaseTasks.length}
                  </Badge>
                </div>
        ...
        <div className="text-center py-16 text-muted-foreground border border-dashed border-border rounded-xl">
          <p className="text-sm">
            Selecione um projeto acima para ver o Kanban
          </p>
        </div>
      )}
    </div>
