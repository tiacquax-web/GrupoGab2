...
export default function Agenda() {
  ...
  const { data: events = [], refetch } = trpc.agenda.list.useQuery({
  ...
  const upcoming = events
    .filter((e: any) => e.status === "agendado" && new Date(e.startAt) >= today)
    .slice(0, 20);
  const past = events
    .filter((e: any) => e.status !== "agendado" || new Date(e.startAt) < today)
    .slice(0, 10);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Seus compromissos e marcações pessoais
          </p>
        </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
              ...
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <SelectContent>
                    {Object.entries(typeConfig).map(([k, v]: any) => (
            ...
            <Card key={event.id} className="border-border opacity-70">
              <CardContent className="p-4 space-y-1">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge
                    className={`text-xs ${statusConfig[event.status]?.color}`}
                  >
